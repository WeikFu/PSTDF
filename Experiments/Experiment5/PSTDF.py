...
class DTForest(nn.Module):
    ...
    def forward(self, x):
        ...
        y_out, hidden = self.bigru(encodes, self.hidden)
        y_out = torch.transpose(y_out, 1, 2)
        # pooling

        gru_out = F.max_pool1d(y_out, y_out.size(2)).squeeze(2)
